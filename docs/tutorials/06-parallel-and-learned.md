...
to cuOpt captures about 80 % of the per-instance headroom with no model at all (estimated from recorded runs); run live
on 80 MIPLIB instances, the pair improves on HiGHS alone by 30 %. Copies of SCIP that differ only in the
...
| gain in primal-dual integral | 5.5 % | 7.5 % | 9.4 % |
...
simple statistics of the problem. At a 10 s budget they close a quarter to a third of the gap to the oracle, and compared
directly neither LightGBM nor Laya is shown to be better than the other; at 30 s none reliably beats always using the
single best strategy. On a problem family left out of training, the fine-tuned Laya held up better than LightGBM. Rule of thumb from the project: **if a simple model is as good,
